...
def crear_diagrama():
    ...
    def caja(x, y, w, h, titulo, metodos):
        rect = patches.Rectangle((x,y), w, h, fc='white', ec='black')
        ax.add_patch(rect)
    ...
    ax.plot([3.0, 3.5], [4.5, 4.5], 'k-') # Particle - Motor
    ax.plot([6.5, 7.0], [4.5, 4.5], 'k-') # Motor - Potential
    ax.plot([5.0, 5.0], [2.0, 3.5], 'k-') # Chemical - Motor (vertical)
